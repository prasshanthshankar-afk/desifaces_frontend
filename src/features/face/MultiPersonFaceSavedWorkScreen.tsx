...
export default function MultiPersonFaceSavedWorkScreen({ storyId }: Props) {
  ...
  const loadSavedFaces = useCallback(async () => {
    if (!token || libraryLoading || savedFaces.length) return;
  ...
  }, [libraryLoading, savedFaces.length, token]);
  ...
  const useSavedFace = useCallback(async (item: SavedFaceItem) => {
    ...
    setError("");
    try {
      const result = await reuseSavedFace(
  ...
  return (
    <View style={styles.safe}>
      {workflow?.current_stage === "face" && (workspace?.participants?.length ?? 0) > 0 ? (
        ...
        >
          <Surface style={styles.reuseBar}>
            <View style={styles.reuseIntro}>
              <Text style={styles.reuseTitle}>Use a saved Face</Text>
              <Text style={styles.reuseMeta}>
                Reuse an identity you already own at no new Face-generation charge.
              </Text>
            </View>
                ...
                return (
                  <View key={participant.participant_id} style={styles.participantAction}>
                    ...
                    {locked ? (
                      ...
                      <CompactButton
                        label="Saved faces"
                        onPress={() => openSavedFaces(participant.participant_id)}
                        disabled={generating}
                      />
                    )}
                  </View>
      ...
      <View style={styles.studioBody}>
        <MultiPersonFaceCohortDenseScreen key={childKey} storyId={storyId} />
      </View>

      <Modal
        visible={Boolean(pickerParticipantId)}
        transparent
        animationType="fade"
        onRequestClose={() => setPickerParticipantId(null)}
      >
            ...
            <View style={styles.modalHeader}>
              <View style={{ flex: 1, minWidth: 0 }}>
                ...
                <Text style={styles.modalMeta}>
                  Select an existing Face to lock it into this character without generating a new one.
                </Text>
              </View>
                ...
                ListEmptyComponent={
                  <View style={styles.emptyBox}>
                    ...
                    <Text style={styles.emptyMeta}>
                      Close this window and use Check Price → Generate only when you need a new identity.
                    </Text>
                  </View>
                }
                renderItem={({ item }) => {
                  ...
                  return (
                    <Pressable
                      ...
                      style={({ pressed }) => [
                        styles.faceCard,
                        pressed && !reuseBusy && styles.pressed,
                        reuseBusy && styles.disabled,
                      ]}
                    >
                      <View style={styles.faceImageWrap}>
                        {uri ? (
                          ...
                          <View style={styles.facePlaceholder}><Text style={styles.facePlaceholderText}>Face</Text></View>
                        )}
                      </View>
                      <Text style={styles.faceTitle} numberOfLines={1}>
                        {clean(item.title) || "Saved Face"}
                      </Text>
...
const styles = StyleSheet.create({
  ...
  reuseBar: { flexDirection: "row", alignItems: "center", gap: 10, padding: 10 },
  reuseIntro: { flex: 1, minWidth: 0 },
  ...
  participantAction: { minWidth: 90, gap: 4, alignItems: "flex-end" },
  participantName: { maxWidth: 110, color: STUDIO.accentText, fontSize: 9, fontWeight: "900" },
  errorText: { color: "#FFC0C6", fontSize: 9, lineHeight: 13, fontWeight: "700", paddingHorizontal: 4, paddingTop: 4 },
  ...
  faceTitle: { color: STUDIO.text, fontSize: 10, fontWeight: "900", marginTop: 7 },
