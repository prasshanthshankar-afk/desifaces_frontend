...
export async function fetchSpendingSummary(token:string,period:SpendingPeriod="month"){
  return read<SpendingSummary>(endpoints.pricing.spending.summary(period),token);
}

export async function fetchSpendingTransactions(token:string,period:SpendingPeriod="year",kind="all",limit=50,offset=0){
  return read<{items:SpendingTransaction[];total:number;limit:number;offset:number}>(
